...
@section('content')
    ...
    <div class="container mt-3">
        ...
        <h2 class="text-center p-1" style="border-top: 1px solid green; background-color:#C6E2B8">{{ $capitulo->referencia }}
        </h2>
        <div class="border rounded text-center mb-2 p-1">
            <center>
                <div class="border rounded m-1" style="width:90%;">
                    <div class="row">
                        <div class="col-3 text-center border small" style="font-weight:bold">
                            Volúmen
                        </div>
                        <div class="col-3 text-center border small" style="font-weight:bold">
                            División
        ...
        <div class="row">
            <div class="col-1 text-center" id="navleft">
                <a href="{{ route('capitulos.show', ['nombre' => $capituloAnterior->referencia]) }}"
                    class="btn btn-personalizado btn-flotante">
            ...
            <div class="col-12" id="contenido">
                @foreach ($capitulo->pericopas as $pericopa)
                    <div class="pericopa">
                        <h2>
                            {{ $pericopa->titulo }} (v.
                            {{ $pericopa->versiculo_inicial }}-{{ $pericopa->versiculo_final }})
                            @if (!empty($pericopa->descripcion))
                                <button class="btn btn-sm float-end" type="button" data-bs-toggle="collapse"
                                    data-bs-target="#collapseDescripcion{{ $loop->index }}" aria-expanded="false"
                                    aria-controls="collapseDescripcion{{ $loop->index }}">
            ...
            <div class="col-1 text-center" id="navright">
                <a href="{{ route('capitulos.show', ['nombre' => $capituloSiguiente->referencia]) }}"
                    class="btn btn-personalizado btn-flotante">
